...
import bookDataRaw from "../data/book.json";
const bookData: Book[] = Array.isArray(bookDataRaw)
  ? (bookDataRaw as any[]).flat(Infinity) as Book[]
  : [];
import NavBar from "../components/NavBar";
import CategoryMenu from "../components/CategoryMenu";


// Define TypeScript interfaces for book and cart item
...
export default function Home() {
  ...
  useEffect(() => {
    ...
    // Normalize book data to ensure every book has an "image" property
    let results = bookData.map(book => {
      // If "image" is missing but "image src" exists, use it
      if (!book.image && (book as any)["image src"]) {
        return { ...book, image: (book as any)["image src"] };
      }
      // If "image" is missing, set a default image path
      if (!book.image) {
        return { ...book, image: "/default-book.png" };
      }
      return book;
    });
    ...
    const sortedResults = sortBooks(results as Book[]);
